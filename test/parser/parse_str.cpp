...
TEST(parser, parse_str_0) {
  ...
  EXPECT_TRUE(std::ranges::equal(std::get<std::string>(ret.lhs), std::string()));
...
TEST(parser, parse_str_15) {
  ...
  EXPECT_TRUE(std::ranges::equal(std::get<std::string>(ret.lhs), std::string("01234567898765")));
  EXPECT_TRUE(ret.type == lang::ParseExpType::Symbol);
}

TEST(parser, parse_str_32) {
  lang::Parser parser;
  const auto ret = parser.parse(std::string(R"(("0123456789876543210123456789876"))"));
