...
ParseExpType get_exp_type(const std::string_view str) {
  ...
  if (type == ParseExpType::OperatorPlus && str.at(1) == ' ') {
    ...
    return type;
             ...
             (static_cast<char>(str.at(0)) == '\\' && static_cast<char>(str.at(1)) == '"')) {
  ...
  } else if (const auto name = ExpFuncs(FunName::Set); str.starts_with(name.str)) {
    return ParseExpType::FuncSet;

  } else if (const auto name = ExpFuncs(FunName::Def); str.starts_with(name.str)) {
...
ParseExp Parser::parse_operator(const std::string_view str, const ParseExpType type) {
  ...
  const auto second_pos =
      is_first_exp ? first_end_paren + 2 : str.find_first_not_of(' ', first_pos + 1);
  ...
  const std::string_view first_view(str.begin() + (is_first_exp ? first_pos + 1 : first_pos),
                                    str.begin() + (is_first_exp ? second_pos - 2 : second_pos - 1));
  auto first = !is_first_exp ? this->parse_int(first_view).val
  ...
  auto second = !is_second_exp ? this->parse_int(second_view).val
