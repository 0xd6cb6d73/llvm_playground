...
namespace lang {
namespace impl {
constexpr std::array<std::string, 6> ExpFunctions{"call", "set", "def", "if", "while", "for"};
}
enum class ParseExpType : std::uint8_t {
  Int,
  Symbol,
  OperatorPlus = '+',
  OperatorMinus = '-',
...
enum class FunName : std::uint8_t { Call = 0, Set = 1, Def = 2, If = 3, While = 4, For = 5 };
class ExpFuncs {
public:
  ExpFuncs(FunName name) { this->str = impl::ExpFunctions.at(static_cast<std::uint8_t>(name)); };
  std::string_view str;
};
struct ParseExp;
using ParseExpVal = std::variant<std::intptr_t, std::string, std::unique_ptr<ParseExp>>;
